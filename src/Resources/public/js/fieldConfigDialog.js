...
opendxp.registerNS("opendxp.plugin.datahub.fieldConfigDialog");
opendxp.plugin.datahub.fieldConfigDialog = Class.create({
    ...
    initialize: function (type, generalConfig, columnConfig, callback, settings) {
        ...
        this.configPanel = new Ext.Panel({
            ...
            title: t("plugin_opendxp_datahub_configpanel_fields"),
            items: [
        ...
        buttons.push({
            text: t("plugin_opendxp_datahub_add_all_definitions"),
        ...
        this.window = new Ext.Window({
            ...
            height: '95%',
            modal: true,
            title: t("plugin_opendxp_datahub_" + this.type) + " " + t('plugin_opendxp_datahub_configpanel_schema_fields') + ' - ' + this.generalConfig.className,
            layout: "fit",
            items: [this.tabPanel],
            buttons: buttons
        });
        ...
        if (!this.selectionPanel) {
            ...
            for (var i = 0; i < this.columnConfig.columns.length; i++) {
                ...
                if (nodeConf.isOperator) {
                    ...
                    var child = {
                        ...
                        iconCls: "opendxp_icon_" + attributes.dataType
                    };
                    if (attributes.width) {
            ...
            var store = new Ext.data.TreeStore({
                ...
                root: {
                    id: "0",
                    root: true,
                    text: t("plugin_opendxp_datahub_configpanel_root"),
                    leaf: false,
                    isTarget: true,
                    expanded: true,
                    children: children
            ...
            this.selectionPanel = new Ext.tree.TreePanel({
                ...
                id: 'tree',
                region: 'east',
                title: t('plugin_opendxp_datahub_configpanel_available_fields'),
                layout: 'fit',
                width: 640,
                split: true,
                autoScroll: true,
    ...
    getOperatorTrees: function () {
        var operators = opendxp.plugin.datahub[this.type + "operator"] ? Object.keys(opendxp.plugin.datahub[this.type + "operator"]) : [];
        var operatorGroups = [];
        ...
        for (var i = 0; i < operators.length; i++) {
            ...
            if (!this.availableOperators || this.availableOperators.indexOf(operator) >= 0) {
                var nodeConfig = opendxp.plugin.datahub[this.type + "operator"][operator].prototype;
    ...
    getConfigElement: function (configAttributes) {
        ...
        if (attributes && attributes.class && attributes.type) {
            ...
            if (opendxp.plugin.datahub[this.type + attributes.type] && opendxp.plugin.datahub[this.type + attributes.type][jsClass]) {
                element = new opendxp.plugin.datahub[this.type + attributes.type][jsClass](this.generalConfig.classId);
            }
        } else {
            var dataType = configAttributes.dataType ? configAttributes.dataType.toLowerCase() : null;
            if (opendxp.plugin.datahub[this.type + "value"] && opendxp.plugin.datahub[this.type + "value"][dataType]) {
                element = new opendxp.plugin.datahub[this.type + "value"][dataType](this.generalConfig.classId);
            } else {
                element = new opendxp.plugin.datahub[this.type + "value"]["defaultvalue"](this.generalConfig.classId);
            }
        }
        return element;
    },
    ...
    checkSupported: function (record, silent = false) {
        if (record.data.type == "data") {
            ...
            if (dataType != "system" && !in_array(dataType, opendxp.plugin.datahub.graphql["supported" + ucfirst(this.type) + "DataTypes"])) {
                if (!silent) {
                    Ext.MessageBox.alert(t("error"), sprintf(t("plugin_opendxp_datahub_" + this.type) + " " + t('plugin_opendxp_datahub_datatype_not_supported_yet'), dataType));
                }
                return false;
            }
        }
